...
function makeService() {
  return {
    getApplicationsForAdmin: jest.fn().mockResolvedValue({ data: [], total: 0 }),
...
describe('AdminApplicationController', () => {
  ...
  it('list() uses defaults and delegates', async () => {
    ...
    expect(service.getApplicationsForAdmin).toHaveBeenCalledWith({ page: 1, limit: 10, q: undefined, status: undefined, penaltyApplied: undefined });
    expect(result).toEqual({ data: [], total: 0 });
  });

  it('list() passes through dto values', async () => {
    await controller.list({ page: 2, limit: 5, q: 'test', status: 'ACCEPTED', penalty_applied: true } as any);
    expect(service.getApplicationsForAdmin).toHaveBeenCalledWith({ page: 2, limit: 5, q: 'test', status: 'ACCEPTED', penaltyApplied: true });
  });
