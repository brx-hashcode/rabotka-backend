import { runVerifyWhatsappFlow, getVerifyWhatsappInitialState } from '../verify-whatsapp.flow';
...
function makeCtx(tokenOverrides: any = null) {
  const mockPrisma: any = {
    verificationToken: {
      findFirst: jest.fn().mockResolvedValue(tokenOverrides === null ? null : { ...mockToken, ...tokenOverrides }),
...
describe('runVerifyWhatsappFlow', () => {
  ...
  it('returns error when token is expired', async () => {
    ...
    const result = await runVerifyWhatsappFlow(makeState(), 'ABC123', profile, ctx);
  ...
  it('handles wallet error gracefully', async () => {
    const ctx = makeCtx({});
    ctx.walletService.grantWelcomeCredit.mockRejectedValue(new Error('Wallet error'));
    ctx.walletService.getOrCreateProfileWallet.mockRejectedValue(new Error('Wallet error'));
    const result = await runVerifyWhatsappFlow(makeState(), 'ABC123', profile, ctx);
    expect(result.clearState).toBe(true);
  });
});
